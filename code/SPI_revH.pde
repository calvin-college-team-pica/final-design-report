...
Connections:
CS: pin 10 :ADE pin 17      // example code suggests CS = pin 7
...
const int currentThresh = 2;		          // set the limit for allowable current
      int voltageData = 0;
      int currentData = 0;

// Any pins needed to connect master & slave
const int slaveSelect = 10;
//const int chipSelect = 7;			  // 7 because what example code used
...
void setup() {
  ...
  SPI.setDataMode( SPI_MODE3 );                   // spi_mode0 (not modeo...)
  //pinMode( chipSelect, OUTPUT );	          // initalize pins specified above, except automatically uses pin 10 so we're going with that while this is commented out
  pinMode( slaveSelect, OUTPUT );                 // digital pot example still uses this
  //pinMode( solidState, OUTPUT );	          // commented out until we can read - might not even need
...
//writeRegister( 0x09, )			  // select sample rates, filter enable & calibration; commented out b/c default looks acceptable, with exception of bits 11,12
  delay( 100 ); 				  // allow ADE to set up as necessary
  
  Serial.print( "Starting Voltage is " );
  Serial.print( voltageData );			  // print to the effective Base Station
  Serial.print( "\n" );
  Serial.print( "Starting Current is " );
  Serial.print( currentData );			  // print to the effective Base Station
  Serial.print( "\n" );
}
...
void loop() {
  ...
  //  voltageData = readRegister( chan1, 3 );
...
//currentData = readRegister( chan2, 3 );
  int currentData = readRegister( 0x16 );
  Serial.print( "\n Current is " );
  Serial.print( currentData );					// print to the 'Base Station'
 // Serial.print( 0bcurrentData );
  Serial.print( "\n" );
  delay( 2000 );
...
// If we're going to use ZigBee to transfer, now would be a good time to CS to a new
// device and spit out the values in voltageData and currentData
}
*/
...
//   Serial.print( "\n Result: " );
//   Serial.print( result );
//  digitalWrite( chipSelect, LOW );                    // put CS low before the read, as specified by the ADE7763 datasheet, commented out until we know what they call CS
  SPI.setDataMode( SPI_MODE3 );                         // mode2 has clock phase "rising"
  digitalWrite( slaveSelect, LOW );
  byte dataToSend = READ | thisRegister;
  int printDataToSend = int( dataToSend );            // this should be the 0 designating a read command in the first the register address
   Serial.print( "\n dataToSend: " );
...
/*   Serial.print( "\n Or-ed: " );
  ...
  // get the third byte & add to the result
...
*/
  SPI.setDataMode( SPI_MODE2 );                        // mode3 has clock phase "falling"
  
  
 //  digitalWrite( chipSelect, HIGH );                   // commented out for same reason as before
  digitalWrite( slaveSelect, HIGH );
  return( second2Result );
}

 
// Read function, as defined by Nathan Jen
   // don't think we need to specify how many bits being read
   // ADE datasheet says will just start shifting until done
/*  unsigned int readRegister( byte thisRegister, int bytesToRead ) {
  int result = 0x1;
  byte inByte = 0;				    // byte coming from SPI - 
  byte dataToSend = READ | thisRegister;	    // combine the read command and the specific register using bitwise OR (not boolean OR...)
  digitalWrite( chipSelect, LOW );		    // select the chip
  SPI.transfer( dataToSend );			    // send the actual command to read
  Serial.print( "bytesToRead: " );
  Serial.print( bytesToRead );                      // debugging purposes
  Serial.print( "\n" );
  bytesToRead--;				    // decrement number of bytes to read
  Serial.print( "decremented bytesToRead: " );
  Serial.print( bytesToRead );                      // debugging purposes
  Serial.print( "\n" );
  ...
  if (bytesToRead > 0) {
    ...
    Serial.print( "shifted result: " );
    Serial.print( result );                         // debugging
    Serial.print( "\n" );
    inByte = SPI.transfer(0x00);                    // not sure why doing this, other than the example code does
    Serial.print( "inByte: " );
    Serial.print( inByte );
    Serial.print( "\n" );
  ...
  return( result );				    // return the value read
}
*/
...
// Taken directly from example code - this is where things are going to get ugly
unsigned int readRegister(byte thisRegister, int bytesToRead ) {
